...
$('#blog-2col-sidebar-form').submit(function () {
	...
	//end
	//get cate_id
	var cate_id=$(".blog-4col-dynamic-more-2").attr("cate_id");
	
	var url=base_url+'json8/'+uid+'/'+cate_id;
		
	$.getJSON(url+'?page=0', function(data) {     
	   
	    if(!data.pager.pages){ 
	       if(valert==1){alert('Bạn chưa có bài viết blog nào ! Dữ liệu bạn đang xem chỉ là demo.');}
	    }
		total=parseInt(data.pager.pages);
		current=parseInt(data.pager.page);
		$('.blog-4col-dynamic-more-2 .blog-item').each(function( index ) {
			if(data.nodes[index] != undefined){
				$(this).find( ".item-title a .title" ).text(data.nodes[index].node.title);
				$(this).find( ".item-title p.date span" ).text(data.nodes[index].node.date);
				$(this).find( ".item-title p.desc" ).text(data.nodes[index].node.content);
				$(this).find( ".item-title p.category" ).text(data.nodes[index].node.category);
...
var button = document.getElementById("view_more_4");
...
function view_more_4(){
	...
	$.getJSON(base_url, function(data) {
	    ...
	    if(parseInt(data.pager.pages)==1){
              $("#view_more_4" ).css('display','none');
        }
	    
	    current=parseInt(data.pager.page);
	    $.each(data.nodes, function(key, value) {
	        clone=$('.blog-4col-dynamic-more-2 .blog-item:last').clone();
          	$(clone).find( ".item-title a .title" ).text(data.nodes[key].node.title);
          	$(clone).find( ".item-title p.date span" ).text(data.nodes[key].node.date);
          	$(clone).find( ".item-title p.desc" ).text(data.nodes[key].node.content);
          	$(clone).find( ".item-title p.category" ).text(data.nodes[key].node.category);
            ...
            $(clone).appendTo( ".blog-4col-dynamic-more-2 div.row" );
...
function getParam(k){
	...
	//end
	//get cate_id
	var cate_id=$(".blog-list-dynamic").attr("cate_id");
	
	var url=base_url+'json/'+uid+'/'+cate_id;
	...
	$.getJSON(url+'?page=0', function(data) {    
		...
		total=parseInt(data.pager.pages);
		current=parseInt(data.pager.page);
		$('.blog-list-dynamic .blog-item').each(function( index ) {
			if(data.nodes[index] != undefined){
				$(this).find( "div h4.title" ).text(data.nodes[index].node.title);
				$(this).find( "div p span.date" ).text(data.nodes[index].node.date);
				$(this).find( "div p span.category" ).text(data.nodes[index].node.category);
				$(this).find( "div p.desc" ).text(data.nodes[index].node.content);
				//$(this).find( "a" ).attr("href",data.nodes[index].node.link);
				$(this).find( "a" ).attr("href",data.nodes[index].node.alias+".html");
				$(this).find( "a .item-img" ).attr("src",data.nodes[index].node.image);
			}
...
function view_more(){
	...
	//var base_url="https://slimweb.vn/json4/"+uid+"?page="+next;
	$.getJSON(base_url, function(data) {
	    current=parseInt(data.pager.page);
	    if(parseInt(data.pager.pages)==1){
	    ...
	    $.each(data.nodes, function(key, value) {
	        clone=$('.blog-list-dynamic .blog-item:last').clone();
          	$(clone).find( "div h4.title" ).text(data.nodes[key].node.title);
          	$(clone).find( "div p span.date" ).text(data.nodes[key].node.date);
          	$(clone).find( "div p span.category" ).text(data.nodes[key].node.category);
          	$(clone).find( "div p.desc" ).text(data.nodes[key].node.content);
            //$(clone).find( "a" ).attr("href",data.nodes[key].node.link);
            $(clone).find( "a" ).attr("href",data.nodes[key].node.alias+".html");
            $(clone).find( "a .item-img" ).attr("src",data.nodes[key].node.image);
            $(clone).appendTo( ".blog-dynamic div.col" );
            if(current + 1 == parseInt(data.pager.pages)){
              $("#view_more" ).css('display','none');
            }
        });
